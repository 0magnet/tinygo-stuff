...
// Configure gpio to drive relays
var relays = []m.Pin{m.GP6, m.GP7, m.GP8, m.GP9, m.GP10, m.GP11, m.GP12, m.GP13}

// Configure multiple HD44780 displays
//var displays = [...]HD44780{{Mode: "clock",Color: hiwht+hiblubg,Speed: time.NewTicker(333 * time.Millisecond), DataPins:    []m.Pin{m.GP22, m.GP21, m.GP20, m.GP19, m.GP18, m.GP17, m.GP16, m.GP15},RS: m.GP26,EN: m.GP27,RW:m.NoPin,Contrast:m.GP28,Clvl:2,Rows:2,Columns:16,CursorBlink:false,CursorOnOff:false,},{Mode: bible,Color: blk+grnbg,Speed: time.NewTicker(333 * time.Millisecond), DataPins:[]m.Pin{m.GP5, m.GP6, m.GP7, m.GP8, m.GP9, m.GP10, m.GP11, m.GP12},RS:m.GP4,EN: m.GP3,RW:m.NoPin,Contrast:m.GP2,Clvl:6,Rows:2,Columns:20,CursorBlink: false,CursorOnOff: false,},}

// Configure multiple HD44780 displays
var displays = [...]HD44780{{Mode: "clock", Color: hiwht + hiblubg, Speed: time.NewTicker(333 * time.Millisecond), DataPins: []m.Pin{m.GP22, m.GP21, m.GP20, m.GP19, m.GP18, m.GP17, m.GP16, m.GP15}, RS: m.GP26, EN: m.GP27, RW: m.NoPin, Contrast: m.GP28, Clvl: 2, Rows: 2, Columns: 16, CursorBlink: false, CursorOnOff: false}}
...
func main() {
	lendisplays := len(displays)
	lenrelays := len(relays)
	...
	if lenrelays > 0 {
		for i, _ := range relays {
			relays[i].Configure(m.PinConfig{Mode: m.PinOutput})
		}
	}

	lcds := make([]*hd44780.Device, lendisplays)
	...
	m.Serial.Write([]byte("\033c"))
	console[len(console)-1] = "\x1b[0m\n\n"
	console[0] = chome + "\033[?25h" + l()
	relayTicker := time.NewTicker(333 * time.Millisecond)
	for {
		if console[0] != chome+"\033[?25h"+l() {
